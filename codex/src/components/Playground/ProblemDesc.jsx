...
function ProblemDesc({ problemId }) {
    ...
    const fetchProblem = async () => {
      try {
        ...
        const response = await axios.get(
          `http://localhost:3010/api/problem/${problemId}`,
          token ? {
            headers: {
        ...
        setProblem(response.data);
        setUserStatus(response.data.status); // From updated controller
      } catch (error) {
        console.error('Error fetching problem:', error);
      }
    };
  ...
  const StatusBadge = () => {
    if (!userStatus || userStatus === 'not_attempted') {
      return <span className="px-3 py-1 bg-gray-200 text-gray-700 rounded-full text-sm">Not Attempted</span>;
    }
    if (userStatus === 'attempted') {
      return <span className="px-3 py-1 bg-yellow-200 text-yellow-800 rounded-full text-sm">Attempted</span>;
    }
    if (userStatus === 'solved') {
      return <span className="px-3 py-1 bg-green-200 text-green-800 rounded-full text-sm">✓ Solved</span>;
    }
  };
  ...
  const DifficultyBadge = () => {
    ...
    return (
      <span className={`px-3 py-1 rounded-full text-sm ${colors[problem.difficulty]}`}>
        {problem.difficulty}
      </span>
    );
  };
  ...
  return (
    <div className="container mx-auto p-6 space-y-6 overflow-auto h-full">
      ...
      <div className="space-y-3">
        ...
        <p className="text-lg text-gray-600 whitespace-pre-line">{problem.description}</p>
      </div>

      {/* Constraints */}
      {problem.constraints && (
        <div className="space-y-2">
          <h3 className="text-xl font-medium text-gray-800">Constraints:</h3>
      ...
      {/* Example Test Cases Section */}
      <div className="space-y-4">
        <h3 className="text-xl font-medium text-gray-800">Example Test Cases:</h3>
        <div className="space-y-3">
          {problem.testCases.map((testCase, index) => (
            <div key={index} className="bg-gray-50 p-4 rounded-lg border space-y-2">
              <div className="font-semibold text-gray-700">Example {index + 1}:</div>
              <div className="space-y-1">
                <div>
                  <span className="font-medium text-gray-700">Input: </span>
                  <code className="bg-gray-200 px-2 py-1 rounded text-sm">{testCase.input}</code>
                </div>
                <div>
                  <span className="font-medium text-gray-700">Output: </span>
                  <code className="bg-gray-200 px-2 py-1 rounded text-sm">{testCase.output}</code>
                </div>
              </div>
            </div>
          ))}
